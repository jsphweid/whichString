...
export function myFft(arr: number[]) {
  return absAndScale(FFT(arr))
}

export function FFT(input: any) {
  return ensureComplexArray(input).FFT();
}

export function InvFFT(input: any) {
...
export function absAndScale(arr: iComplexArray): any[] {
    const final = []
    for (let i = 0; i < arr.length; i++) {
        final.push(2 * Math.sqrt((arr.real[i] * arr.real[i]) + (arr.imag[i] * arr.imag[i])))
    }
    return final
}
...
function ensureComplexArray(input: any) {
  return input instanceof ComplexArray && input || new ComplexArray(input);
}

function fft(input: any, inverse: any) {
...
function FFT_Recursive(input: any, inverse: any) {
  const n = input.length;
...
function FFT_2_Iterative(input: any, inverse: any) {
  const n = input.length;
...
function BitReverseIndex(index: any, n: any) {
...
function BitReverseComplexArray(array: any) {
...
function LowestOddFactor(n: any) {
